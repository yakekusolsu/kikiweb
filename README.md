...
Bot接続中は`KIKIWEB_SITE_STATUS`の文言をWebサイトに表示します。Discord側のVCステータスは変更しないため、
Botに「ボイスチャンネルステータスを設定」権限は不要です。従来の`KIKIWEB_VOICE_STATUS`もサイト表示用として利用できます。
...
最新版Botは参加中のDiscordサーバー総数もRelayへ通知し、Webの「導入サーバー」に表示します。
